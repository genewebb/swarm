...
14. **Pre-handoff gates** — apply these checks *before* writing any handoff file. Only after all applicable gates pass may you proceed to write the handoff:
    - **Decomposition confirmation gate**: If the next agent is `plan-decomposer`, apply the full gate described in the "Decomposition confirmation gate" section above. This gate may stop the workflow awaiting user input.
